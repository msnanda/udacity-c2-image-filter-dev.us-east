...
(async () => {
  ...
  app.get( "/", async ( req: Request, res:Response ) => {
    res.send("provide the image_url and use endpoint /filteredimage?image_url='the url'")
  } );
   ...
   app.get("/filteredimage", async ( req: Request, res:Response ) => {
      ...
      if (!image_url) {
        return res.status(400).send("Please provide the query parameter image_url")
      }
